...
public final class DolphinJobDispatcher implements JobDispatcher {
  ...
  @Inject
  private DolphinJobDispatcher(final ETMaster etMaster,
  ...
  public void executeJob(final JobEntity jobEntity) {
    ...
    final Future<List<AllocatedExecutor>> serversFuture = etMaster.addExecutors(dolphinJobEntity.getNumServers(),
          dolphinJobEntity.getServerExecutorConf());
    final Future<List<AllocatedExecutor>> workersFuture = etMaster.addExecutors(dolphinJobEntity.getNumWorkers(),
          dolphinJobEntity.getWorkerExecutorConf());
    ...
    new Thread(() -> {
      try {
        ...
        final Future<AllocatedTable> modelTableFuture =
            etMaster.createTable(dolphinJobEntity.getServerTableConf(), servers);
        final Future<AllocatedTable> inputTableFuture =
            etMaster.createTable(dolphinJobEntity.getWorkerTableConf(), workers);
