...
public interface Trainer<K, V> {
  ...
  /**
   * Set training data for mini-batch.
   * @param miniBatchTrainingData the training data to process in the mini-batch
   */
  void setMiniBatchData(Collection<Map.Entry<K, V>> miniBatchTrainingData);

  /**
   * Pull model for training the mini-batch data set by {@link #setMiniBatchData}.
   */
  void pullModel();

  /**
   * Perform local computation of mini-batch based on the model pulled by {@link #pullModel}.
   */
  void localCompute();

  /**
   * Push updates generated by {@link #localCompute}.
   */
  void pushUpdate();
